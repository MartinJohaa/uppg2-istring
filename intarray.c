...
int istrcmp(const char *s1, const char *s2){
    ...
    if(istrlen(s2) > istrlen(s1)) return -1;
    if(istrlen(s1) > istrlen(s2)) return 1;
    else return 0;
}
size_t istrlen(const char *s){
  STRING(s);
